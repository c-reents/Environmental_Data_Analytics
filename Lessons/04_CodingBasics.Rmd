...
* **Vectors** 1 dimensional structure that contains elements of the same type (ex: all numbers, all letters).

* **Matrices** 2 dimensional structure that contains elements of the same type.(most of what we will talk about will be 2-d)
...
vector1[3] # gives the third item of the vector (the third element of vector1) - matrix subsetting

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # matrix of numbers 1:20 with 5 rows and 4 columns, deafult- automatically counts vertically first
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #counts horizontally first then vertically 
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), 
                                  c("un", "deux", "trois", "cat"))) #naming rows then columns
matrix3

#indexing
matrix1[4, ] # pull items from row 4
matrix1[ , 3] # pull items from column 3 [row, column]
matrix1[c(12, 14)] # pull element 12 and 14 (need the c or it will think you want it to pull row 12, column 14 - c means "call")
matrix1[c(12:14)] # pull elements 12 through 14
matrix1[2:4, 1:3] # create matrix with only rows 2:4 and columns 1:3
matrix1[,1:3]
...
# Lists ---- 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1 #naming the object within the code rather than defining them seperately 
list1[[2]]
...
dataframe1 <- data.frame(d,e,f) #combine these vectors into a matrix
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # View() pulls up the visualization in a seperate window 

dataframe1[1:2] # showing columns 1 and 2 because it is a dataframe not a matrix, if it were a matrix it would give elements 1 and 2
...
> ANSWER: they have different descriptive statements, lists, matrices and data frames go under data. Vectors go under values.

QUESTION: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> ANSWER: The first element from the list of data elements appears 

QUESTION: What happens when a comment in R is followed by "----"?

> ANSWER:it becomes a collapsible section
...
2. Type `install.packages("packagename")` into your R chunk or console (do this in the console so that you dont have the install.packages function in your script a) because you only need to do this once b)it will cause knitting problems). It will then appear in your packages list. You only need to do this once. 
...
* Update your packages regularly! go to packages tab and hit update
