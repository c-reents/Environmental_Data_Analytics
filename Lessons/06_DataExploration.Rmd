...
str(USGS.flow.data) #shows structure
dim(USGS.flow.data) #shows dimensions

#head(USGS.flow.data) #shows first 6
...
**format dates as Y/M/D especiaally important in international settings
...
format(today, format = "%m") # month as a number
format(today, format = "%a") #abbreviated day
format(today, format = "%A") #unabbreviated day
...
#paste0 takes the two statements in the code together. If date is greater than 181231 (2018-12-31) put "19", if not "20", ammend that to d
create.early.dates <- (function(d) {
       paste0(ifelse(d > 181231,"19","20"),d)
       })
# run code on the dates
...
> Answer: NAs sometimes mean something--> in psychology or social sciences a non-answer is an answer(qualitative research or survey analytics), quantitative data can only be analyzed without NAs. You must always state whether or not you have removed NAs. Pay attention to true NAs versus missing data

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data)
dim(USGS.flow.data)
dim(USGS.flow.data.complete)
#complete.cases() will tell you which cases are complete
...
```{r, fig.height = 3, fig.width = 4}
# creates a warning message telling you it will be using 30 bins
...
```
### Frequency line graph (function: geom_freqpoly) --> alternative to your histogram, visualizing distributions of data
...
```{r, fig.height = 3, fig.width = 4}
# useful to have things in similar units , color is outside of the aes
ggplot(USGS.flow.data.complete) +
...
#color based on approval status (color is in the aes() )
ggplot(USGS.flow.data.complete) +
